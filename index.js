...
// Create a new reservation (Admin only), isAdmin
app.post('/reservations', async (req, res) => {
    const { totalGuests, resDate, timeFrame, availTables, occupied, availCapacity, fname, lname, email, phone, comment, combineTbl } = req.body;

    const newReservation = new TableAudit({
        totalGuests,
        resDate,
        timeFrame,
        availTables,
...
// Read all reservations (Admin only) 
app.get('/reservations',  async (req, res) => {
    try {
        const reservations = await TableAudit.find();
...
// Update a reservation (Admin only) , isAdmin
app.put('/reservations/:id', async (req, res) => {
    const { id } = req.params;
    const updates = req.body;

    try {
        const reservation = await TableAudit.findByIdAndUpdate(id, updates, { new: true });
...
// Delete a reservation (Admin only) , isAdmin
app.delete('/reservations/:id', async (req, res) => {
    const { id } = req.params;

    try {
        await TableAudit.findByIdAndDelete(id);
